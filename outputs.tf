...
output "service_split_traffic" {
  value = try(google_app_engine_service_split_traffic.this.*.id)
}

output "service_network_settings" {
  value = try(google_app_engine_service_network_settings.this.*.id)
}
...
output "firewall_rules" {
  value = try(google_app_engine_firewall_rule.this.*.id)
}

output "domain_mapping" {
  value = try(google_app_engine_domain_mapping.this.*.id)
}

output "url_dispatch_rules" {
  value = try(google_app_engine_application_url_dispatch_rules.this.*.id)
}
...
variable "application" {
  ...
  validation {
    condition     = alltrue([for a in var.application : true if contains(["CLOUD_FIRESTORE", "CLOUD_DATASTORE_COMPATIBILITY"], a.database_type)])
...
variable "domain_mapping" {
  ...
  validation {
    condition     = alltrue([for a in var.domain_mapping : true if contains(["STRICT", "OVERRIDE"], a.override_strategy)])
    error_message = "Valid values are STRICT or OVERRIDE."
  }

  validation {
    condition     = alltrue([for b in var.domain_mapping : true if contains(["AUTOMATIC", "MANUAL"], b.ssl_settings.ssl_management_type)])
...
variable "firewall_rule" {
  ...
  validation {
    condition     = alltrue([for a in var.firewall_rule : true if contains(["UNSPECIFIED_ACTION", "ALLOW", "DENY"], a.action)])
...
variable "flexible_app_version" {
  ...
  validation {
    condition     = alltrue([for a in var.flexible_app_version : true if contains(["INBOUND_SERVICE_MAIL", "INBOUND_SERVICE_MAIL_BOUNCE", "INBOUND_SERVICE_XMPP_ERROR", "INBOUND_SERVICE_XMPP_MESSAGE", "INBOUND_SERVICE_XMPP_SUBSCRIBE", "INBOUND_SERVICE_XMPP_PRESENCE", "INBOUND_SERVICE_CHANNEL_PRESENCE", "INBOUND_SERVICE_WARMUP"], a.inbound_services)])
  ...
  validation {
    condition     = alltrue([for b in var.flexible_app_version : true if contains(["F1", "F2", "F4", "F4_1G", "B1", "B2", " B4", "B8", "B4_1G"], b.instance_class)])
    error_message = "Valid values are AutomaticScaling: F1, F2, F4, F4_1G ManualScaling: B1, B2, B4, B8, B4_1G."
  }

  validation {
    condition     = alltrue([for c in var.flexible_app_version : true if contains(["SERVING", "STOPPED"], c.serving_status)])
    error_message = " Possible values are: SERVING, STOPPED."
  }

  validation {
    condition     = alltrue([for d in var.flexible_app_version : true if contains(["python", "java", "php", "ruby", "go", "nodejs"], d.runtime_api_version)])
  ...
  validation {
    condition     = alltrue([for e in var.flexible_app_version : true if contains(["SECURE_DEFAULT", "SECURE_NEVER", "SECURE_OPTIONAL", "SECURE_ALWAYS"], e.handlers.security_level)])
    error_message = "Possible values are: SECURE_DEFAULT, SECURE_NEVER, SECURE_OPTIONAL, SECURE_ALWAYS."
  }

  validation {
    condition     = alltrue([for f in var.flexible_app_version : true if contains(["LOGIN_OPTIONAL", "LOGIN_ADMIN", "LOGIN_REQUIRED"], f.handlers.login)])
    error_message = "Possible values are: LOGIN_OPTIONAL, LOGIN_ADMIN, LOGIN_REQUIRED."
  }

  validation {
    condition     = alltrue([for g in var.flexible_app_version : true if contains(["AUTH_FAIL_ACTION_REDIRECT", "AUTH_FAIL_ACTION_UNAUTHORIZED"], g.handlers.auth_fail_action)])
    error_message = "Possible values are: AUTH_FAIL_ACTION_REDIRECT, AUTH_FAIL_ACTION_UNAUTHORIZED."
  }

  validation {
    condition     = alltrue([for h in var.flexible_app_version : true if contains(["REDIRECT_HTTP_RESPONSE_CODE_301", "REDIRECT_HTTP_RESPONSE_CODE_302", "REDIRECT_HTTP_RESPONSE_CODE_303", "REDIRECT_HTTP_RESPONSE_CODE_307"], h.handlers.redirect_http_response_code)])
  ...
  validation {
    condition     = alltrue([for i in var.flexible_app_version : true if contains(["LOGIN_OPTIONAL", "LOGIN_ADMIN", "LOGIN_REQUIRED"], i.api_config.login)])
    error_message = "Possible values are: LOGIN_OPTIONAL, LOGIN_ADMIN, LOGIN_REQUIRED."
  }

  validation {
    condition     = alltrue([for j in var.flexible_app_version : true if contains(["AUTH_FAIL_ACTION_REDIRECT", "AUTH_FAIL_ACTION_UNAUTHORIZED"], j.api_config.auth_fail_action)])
    error_message = "Possible values are: AUTH_FAIL_ACTION_REDIRECT, AUTH_FAIL_ACTION_UNAUTHORIZED."
  }

  validation {
    condition     = alltrue([for k in var.flexible_app_version : true if contains(["SECURE_DEFAULT", "SECURE_NEVER", "SECURE_OPTIONAL", "SECURE_ALWAYS"], k.api_config.security_level)])
    error_message = "Possible values are: SECURE_DEFAULT, SECURE_NEVER, SECURE_OPTIONAL, SECURE_ALWAYS."
  }

  validation {
    condition     = alltrue([for l in var.flexible_app_version : true if contains(["FIXED", "MANAGED"], l.endpoints_api_service.rollout_strategy)])
...
variable "service_network_settings" {
  ...
  validation {
    condition     = alltrue([for a in var.service_network_settings : true if contains(["INGRESS_TRAFFIC_ALLOWED_UNSPECIFIED", "INGRESS_TRAFFIC_ALLOWED_ALL", "INGRESS_TRAFFIC_ALLOWED_INTERNAL_ONLY", "INGRESS_TRAFFIC_ALLOWED_INTERNAL_AND_LB"], a.ingress_traffic_allowed)])
...
variable "service_split_traffic" {
  ...
  validation {
    condition     = alltrue([for a in var.service_split_traffic : true if contains(["UNSPECIFIED", "COOKIE", "IP", "RANDOM"], a.split.shard_by)])
